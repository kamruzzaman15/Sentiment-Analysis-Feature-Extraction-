...
if __name__=='__main__':
    with EmissionsTracker() as tracker:
        ...
        labels = dataset['positive_review'].values
        ...
        x = Conv1D(filters=128, kernel_size=3, activation='relu',name='feature_layer')(x)
        ...
        full_model.fit(train_data, train_labels, epochs=1, batch_size=32)
        ...
        print('Accuracy for voting product:', acc)

        et = time.time()
        elapsed_time = et - st
        print('Execution time for vot is:', elapsed_time, 'seconds')
        current, peak = tracemalloc.get_traced_memory()
        tracemalloc.stop()
        print("Current memory usage for vot is", current / (1024 * 1024), "MB; Peak was", peak / (1024 * 1024), "MB")

     
    print('Co2 for vot:', tracker.final_emissions)
